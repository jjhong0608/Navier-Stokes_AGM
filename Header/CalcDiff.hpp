...
void SettingDiffCoefficient (Point *pt, xData *xdat, yData *ydat,
  ...
  const double mpy1, const double mpy2) {
    ...
    if (::SolverType) SettingDiffTimeIntegation (HeadVelocity (pt), xdat, ydat, xm, xb, xp, ym, yb, yp, bdx, bdy, mpx1, mpx2, mpy1, mpy2);
}
...
void SettingDiffDiffCoefficient (Point *pt, xData *xdat, yData *ydat,
  ...
  const double mpy1, const double mpy2) {
    ...
    if (::SolverType) SettingDiffDiffTimeIntegation (HeadVelocity (pt), xdat, ydat, xm, xb, xp, ym, yb, yp, bdx, bdy, mpx1, mpx2, mpy1, mpy2);
}
...
double CalcDiff (char xy, Point *pt, xData *xdat, yData *ydat) {
  ...
  for (const auto &i : azimuth) {
    idx++;
    if (HeadVelocity (pt)->EWNS (i[0], i[1])) arrVal[idx] = HeadVelocity (pt)->EWNS (i[0], i[1])->Value (), arrVal[idx + 13] = HeadVelocity (pt)->EWNS (i[0], i[1])->Phi ()->Value ();
  }
...
//  }
  ...
  if (returnval != returnval) {
    for (const auto &i : {'E', 'W', 'N', 'S'}) {
      if (HeadVelocity (pt)->EWNS (i, i)) if (HeadVelocity (pt)->EWNS (i, i)->Condition () == 'C') return HeadVelocity (pt)->EWNS (i, i)->Diff (xy)->Value ();
    }
  }
...
double CalcDiffDiff (char xy, Point *pt, xData *xdat, yData *ydat) {
  ...
  CalcRepresenCoef (pt, xdat, ydat, true);
  // 현재점에서의 해의 값
  arrVal[0 ] = HeadVelocity (pt)->Value (), arrVal[13] = HeadVelocity (pt)->Phi ()->Value (), arrVal[26] = HeadVelocity (pt)->Diff (xy)->Value ();
  for (const auto &i : azimuth) {
    idx++;
    if (HeadVelocity (pt)->EWNS (i[0], i[1])) arrVal[idx     ] = HeadVelocity (pt)->EWNS (i[0], i[1])->Value (),
  ...
  if (xy == 'X' || xy == 'x') {
    ...
    if (::SolverType) returnval += (5.0E-1 * (HeadVelocity (pt)->Value () - HeadVelocity (pt)->Pre ()->Value ()) / HeadVelocity (pt)->Dt ()) / HeadVelocity (pt)->MaterialProperty ();
    arrEnt[0 ] = xdat->Cu,   arrEnt[1 ] = xdat->Eu,   arrEnt[2 ] = xdat->Wu,   arrEnt[5 ] = xdat->ENu,   arrEnt[6 ] = xdat->ESu,   arrEnt[7 ] = xdat->WNu,   arrEnt[8 ] = xdat->WSu,
    arrEnt[13] = xdat->Cphi, arrEnt[14] = xdat->Ephi, arrEnt[15] = xdat->Wphi, arrEnt[18] = xdat->ENphi, arrEnt[19] = xdat->ESphi, arrEnt[20] = xdat->WNphi, arrEnt[21] = xdat->WSphi,
...
//  }
  ...
  if (isnan(returnval)) {
    for (const auto &i : {'E', 'W', 'N', 'S'}) {
      if (HeadVelocity (pt)->EWNS (i, i)) if (HeadVelocity (pt)->EWNS (i, i)->Condition () == 'C') return HeadVelocity (pt)->EWNS (i, i)->Diff (xy)->Diff (xy)->Value ();
    }
  }
