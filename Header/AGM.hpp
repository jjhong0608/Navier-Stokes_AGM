...
int AGM (string AGL_output_file, string AGM_output_file, double initialTime, double terminalTime, int timeStep, double dt) {
  // AGM solver의 시작을 알림
  printf ("+"); for (size_t i = 0; i < 35; i ++) printf ("-"); printf ("+");
  ...
  SettingPoints (&adat, ptU, ptV, ptP, pt_diff_u_x, pt_diff_u_y, pt_diff_v_x, pt_diff_v_y, pt_diff_u_xx, pt_diff_u_yy, pt_diff_v_xx, pt_diff_v_yy, pt_hat_u, pt_hat_v, pt_pre_u, pt_pre_v, pt_old_u, pt_old_v, pt_phi, pt_psi);
  ...
  for (size_t i = 0; i < adat.Pts_Num (); i++) {
    ...
    ptP[i].SetNormal ('x', adat.Normal (i, 'x')); ptP[i].SetNormal ('y', adat.Normal (i, 'y'));
  ...
  for (size_t i = 0; i < adat.Pts_Num (); i++) {
    ...
    // ptU[i].SetValue (adat.Boundaryvalue (i));
    ptU[i].SetValue (u_ftn_Dirichlet (&ptU[i]));
    // v값을 저장하는 점들의 값을 초기화 (경계조건의 값으로 초기화, 내부점인 경우 0으로 초기화)
    // ptV[i].SetValue (adat.Boundaryvalue (i));
    ptV[i].SetValue (u_ftn_Dirichlet (&ptV[i]));

    // u값을 저장하는 점들의우변의 f값을 읽어들임
    // ptU[i].SetF (ptU[i].CalcRightHandSide ());
    ptU[i].SetF (f_ftn (&ptU[i]));
    // v값을 저장하는 점들의 우변의 f값을 읽어들임
    // ptV[i].SetF (ptV[i].CalcRightHandSide ());
    ptV[i].SetF (f_ftn (&ptV[i]));

    // u값을 저장하는 점들의 conductivity를 읽어들임
    ptU[i].SetMaterialProperty (ptP[i].MaterialProperty ());
    // v값을 저장하는 점들의 conductivity를 읽어들임
    ptV[i].SetMaterialProperty (ptP[i].MaterialProperty ());

    // 각 점의 normal vector를 읽어들임
    ptU[i].SetNormal ('x', adat.Normal (i, 'x')); ptU[i].SetNormal ('y', adat.Normal (i, 'y'));
    ptV[i].SetNormal ('x', adat.Normal (i, 'x')); ptV[i].SetNormal ('y', adat.Normal (i, 'y'));
  ...
  // SettingSolver ("Heat"), Solver (&cdat, &adat, ptV);
   ...
   while (ptU[0].Time () < cdat.TerminalTime () - 5.0E-1 * cdat.Dt ()) {
     ...
     for (size_t i = 0; i < adat.Pts_Num (); i++) {
       ...
       // ptV[i].Pre ()->SetValue (ptV[i].Value ());
  ...
  for (size_t i = 0; i < adat.Pts_Num (); i++) {
      ...
      pt_diff_u[i].SetValue (sqrt (pt_diff_u_x[i].Value () * pt_diff_u_x[i].Value () + pt_diff_u_y[i].Value () * pt_diff_u_y[i].Value ()));
  }
  ...
  for (size_t i = 0; i < adat.Pts_Num (); i++) {
      ...
      pt_diff_diff_u[i].SetValue (sqrt (pt_diff_u_xx[i].Value () * pt_diff_u_xx[i].Value () + pt_diff_u_yy[i].Value () * pt_diff_u_yy[i].Value ()));
  }
