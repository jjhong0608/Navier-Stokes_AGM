...
void SettingSolver(string Solvertype) {
    ...
    if (!Solvertype.compare("ELLIPTIC")) ::SolverType = 0;
        ...
        sprintf(errorMassage, "SettingSolver, Solvertype = \"%s\" is wrong", Solvertype.c_str()), PrintError(
                errorMassage);
...
void sortPts(AxialData *adat, Point *pt) {
    ...
    for (size_t i = 0; i < adat->Pts_Num(); i++) {
        // Interface위의 점인 경우
        if (pt[i].Condition() == 'I' || pt[i].Condition() == 'M' || pt[i].Condition() == 'N' ||
            pt[i].Condition() == 'D') {
            // 내부점의 index에서 모든점의 index
            adat->SetPtsTOpts('I', j, i);
            // 모든점의 index에서 내부점이 index
...
double Calc_Error(AxialData *adat, Point *pts) {
    ...
    for (size_t i = 0; i < adat->Pts_Num(); i++) {
        ...
        if (fabs(pts[i].Value() - u_ftn_Dirichlet(&pts[i])) > norm2) {
            norm2 = fabs(pts[i].Value() - u_ftn_Dirichlet(&pts[i]));
            MaxErrorIndex = pts[i].Index();
        }
        ...
        error2 += 0.25 * (pts[i].Value() - u_ftn_Dirichlet(&pts[i])) * (pts[i].Value() - u_ftn_Dirichlet(&pts[i])) *
                  (xp - xm) * (yp - ym);
    }
    // 줄바꿈
    printf("\n");
...
double Calc_Derivative_Error(AxialData *adat, Point *pts) {
    ...
    for (size_t i = 0; i < adat->Pts_Num(); i++) {
        // 참조하는 점이 Dirichlet 경계점이거나 Neumann 경계점이거나 Infinity 점이거나 Singularity점인 경우 pass함
        if (pts[i].Condition() == 'D' || pts[i].Condition() == 'N' || pts[i].Condition() == 'F' ||
            pts[i].Condition() == 'S')
            continue;
        // 참조하는 점의 x-좌표
        xb = HeadVelocity(&pts[i])->Coord().Value('x');
        ...
        // 미분의 크기를 저장
        Du = sqrt(pts[i].Diff('x')->Value() * pts[i].Diff('x')->Value() +
                  pts[i].Diff('y')->Value() * pts[i].Diff('y')->Value());
        // 참조하는 점에서의 exact solution의 값이 저장한 exact solution의 최대값보다 더 큰 경우, 대체해서 저장
        if (fabs(Du_ftn) > norm1) norm1 = fabs(Du_ftn);
        // 참조하는 점에서의 error가 저장한 error의 최대값보다 더 큰 경우, 대체해서 저장
        if (fabs(Du - Du_ftn) > norm2) {
            norm2 = fabs(Du - Du_ftn);
            MaxErrorIndex = pts[i].Index();
        }
        // 참조하는 점에서의 exact solution의 L2-norm을 저장한 L2-norm에 더함
...
double Calc_Undetermined_Coefficient_in_Unbounded_Domian(AxialData *adat, Point *pt) {
    ...
    std::function<double(double, double)> u1_infinity = [&](double x, double y) {
        return 4.0E0 * log(1.0E0 / sqrt(x * x + y * y));
    };
    ...
    for (size_t i = 0; i < adat->Pts_Num(); i++) {
        ...
        if (pt[i].EWNS('E', 'E') != NULL)
            if (pt[i].EWNS('E', 'E')->Condition() == 'F') {
                // 왼쪽점의 x-좌표를 저장
        ...
        if (pt[i].EWNS('W', 'W') != NULL)
            if (pt[i].EWNS('W', 'W')->Condition() == 'F') {
                // 오른쪽점의 x-좌표를 저장
        ...
        if (pt[i].EWNS('N', 'N') != NULL)
            if (pt[i].EWNS('N', 'N')->Condition() == 'F') {
                // 아래쪽점의 x-좌표를 저장
        ...
        if (pt[i].EWNS('S', 'S') != NULL)
            if (pt[i].EWNS('S', 'S')->Condition() == 'F') {
                // 위쪽점의 x-좌표를 저장
...
void PrintError(const char *massage) {
    // massage: 덧붙일 에러 메시지
    printf("\n");
    for (size_t i = 0; i < 28; i++) printf("%c", '=');
    printf("%s", "Some error has occured");
    for (size_t i = 0; i < 28; i++) printf("%c", '=');
    printf("\n");
    printf("\t%s%s\n", "Error in ", massage);
    for (size_t i = 0; i < 78; i++) printf("%c", '=');
    printf("\n");
    printf("\n");
...
void SettingPoints(AxialData *adat, Point *ptU, Point *ptV, Point *ptP,
    ...
    for (size_t i = 0; i < adat->Pts_Num(); i++) {
        ...
        // 이전의 속도 u의 x성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_u_x[i].SetMark("preux");
        // 이전의 속도 u의 y성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_u_y[i].SetMark("preuy");
        // 이전의 속도 u의 x성분으로의 미분의 x성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_u_xx[i].SetMark("preuxx");
        // 이전의 속도 u의 y성분으로의 미분의 y성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_u_yy[i].SetMark("preuyy");
        // 이전의 속도 v의 x성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_v_x[i].SetMark("prevx");
        // 이전의 속도 v의 y성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_v_y[i].SetMark("prevy");
        // 이전의 속도 v의 x성분으로의 미분의 x성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_v_xx[i].SetMark("prevxx");
        // 이전의 속도 v의 y성분으로의 미분의 y성분으로의 미분을 저장하는 점의 표시
        pt_pre_diff_v_yy[i].SetMark("prevyy");
        // phi를 저장하는 점의 표시
        ...
        pt_pre_diff_u_x[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_u_y[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_u_xx[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_u_yy[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_v_x[i].SetVelocity('v', &ptV[i]);
        pt_pre_diff_v_y[i].SetVelocity('v', &ptV[i]);
        pt_pre_diff_v_xx[i].SetVelocity('v', &ptV[i]);
        ...
        pt_pre_diff_u_y[i].SetVelocity('v', &ptV[i]);
        pt_pre_diff_u_xx[i].SetVelocity('v', &ptV[i]);
        pt_pre_diff_u_yy[i].SetVelocity('v', &ptV[i]);
        pt_pre_diff_v_x[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_v_y[i].SetVelocity('u', &ptU[i]);
        pt_pre_diff_v_xx[i].SetVelocity('u', &ptU[i]);
        ...
        pt_pre_diff_u_x[i].SetPressure(&ptP[i]);
        pt_pre_diff_u_y[i].SetPressure(&ptP[i]);
        pt_pre_diff_u_xx[i].SetPressure(&ptP[i]);
        pt_pre_diff_u_yy[i].SetPressure(&ptP[i]);
        pt_pre_diff_v_x[i].SetPressure(&ptP[i]);
        pt_pre_diff_v_y[i].SetPressure(&ptP[i]);
        pt_pre_diff_v_xx[i].SetPressure(&ptP[i]);
        ...
        // 이전의 속도의 미분을 저장
        pt_diff_u_x[i].SetPre(&pt_pre_diff_u_x[i]);
        pt_diff_u_y[i].SetPre(&pt_pre_diff_u_y[i]);
        pt_diff_u_xx[i].SetPre(&pt_pre_diff_u_xx[i]);
        pt_diff_u_yy[i].SetPre(&pt_pre_diff_u_yy[i]);

        pt_diff_v_x[i].SetPre(&pt_pre_diff_v_x[i]);
        pt_diff_v_y[i].SetPre(&pt_pre_diff_v_y[i]);
        pt_diff_v_xx[i].SetPre(&pt_pre_diff_v_xx[i]);
        pt_diff_v_yy[i].SetPre(&pt_pre_diff_v_yy[i]);
