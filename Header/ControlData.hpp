...
// control data 읽기
ControlData &
ControlData::LoadCtrlData(string AGL_output_file, string AGM_output_file, double initialTime, double terminalTime,
                          int timeStep, double dt) {
    // output file 이름
    string output;

    // Axial line generator에서 만들어진 파일의 이름
    this->AxialFile = std::move(AGL_output_file);
    // output file의 이름
    output = std::move(AGM_output_file);
    // solution파일의 이름
