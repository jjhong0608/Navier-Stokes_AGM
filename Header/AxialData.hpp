...
AxialData::~AxialData() {
    ...
    for (size_t i = 0; i < pts_num; i++) delete[] axial_index[i];
    delete[] axial_index;
    // 점의 왼쪽, 오른쪽, 위쪽, 아래쪽의 점의 index를 저장하는 변수
    for (size_t i = 0; i < pts_num; i++) delete[] EWNS_index[i];
    delete[] EWNS_index;
    // 점의 좌표를 저장하는 변수
    for (size_t i = 0; i < pts_num; i++) delete[] pts[i];
    delete[] pts;
    // x-축선의 정보를 저장하는 변수
    for (size_t i = 0; i < xxaxial_num; i++) delete[] xaxial[i];
    delete[] xaxial;
    // y-축선의 정보를 저장하는 변수
    for (size_t i = 0; i < yyaxial_num; i++) delete[] yaxial[i];
    delete[] yaxial;
...
double AxialData::XYaxial(char xy, int i, int j) {
    ...
    if (xy == 'x' || xy == 'X') {
        ...
        if (i >= xxaxial_num) {
            sprintf(buf, "AxialData::XYaxial, i (= %d) is greater than xxaxial_num (= %d)", i,
                    this->XXYYaxial_Num('x'));
            PrintError(buf);
        }
    ...
    if (xy == 'y' || xy == 'Y') {
        ...
        if (i >= yyaxial_num) {
            sprintf(buf, "AxialData::XYaxial, i (= %d) is greater than yyaxial_num (= %d)", i,
                    this->XXYYaxial_Num('y'));
            PrintError(buf);
        }
        // 잘못된 index인 경우 에러메시지를 출력
        if (j > 3) {
...
/* x-축선과 y-축선위의 점의 개수 */
int AxialData::XYaxial_Num(char xy) const {
...
/* x-축선과 y-축선의 개수 */
int AxialData::XXYYaxial_Num(char xy) const {
    // x-축선의 개수를 return
...
int AxialData::XYaxial_Index(char xy, int i) {
    ...
    if (xy == 'x' || xy == 'X') {
        ...
        if (i >= xaxial_num) {
            sprintf(buf, "AxialData::XYaxial_Index, i (= %d) is greater than xaxial_num (= %d)", i,
                    this->XYaxial_Num('x'));
            PrintError(buf);
        }
    ...
    if (xy == 'y' || xy == 'Y') {
        ...
        if (i >= yaxial_num) {
            sprintf(buf, "AxialData::XYaxial_Index, i (= %d) is greater than yaxial_num (= %d)", i,
                    this->XYaxial_Num('y'));
            PrintError(buf);
        }
...
int AxialData::XXYYaxial_Index(char xy, int i) {
    ...
    if (xy == 'x' || xy == 'X') {
        ...
        if (i > xxaxial_num) {
            sprintf(buf, "AxialData::XXYYaxial_Index, i (= %d) is greater than xxaxial_num (= %d)", i,
                    this->XXYYaxial_Num('x'));
            PrintError(buf);
        }
    ...
    if (xy == 'y' || xy == 'Y') {
        ...
        if (i > yyaxial_num) {
            sprintf(buf, "AxialData::XXYYaxial_Index, i (= %d) is greater than yyaxial_num (= %d)", i,
                    this->XXYYaxial_Num('y'));
            PrintError(buf);
        }
...
AxialData &AxialData::ExportAxialData(ControlData *cdat) {
    ...
    if (yaxial_index_output == nullptr) {
        printf("=========== Some error has occured ==========\n");
    ...
    if (xxaxial_index_output == nullptr) {
        printf("=========== Some error has occured ===========\n");
    ...
    if (yyaxial_index_output == nullptr) {
        printf("=========== Some error has occured ===========\n");
        printf("     Failed to open \"xxaxial_index.dat\"     \n");
        printf("==============================================\n");
        exit(1);
    ...
    if (ptsTOphi_pts_output == nullptr) {
        printf("========== Some error has occured ==========\n");
        printf("     Failed to open \"ptsTOin_pts.dat\"     \n");
        printf("============================================\n");
        exit(1);
    ...
    if (phi_ptsTOpts_output == nullptr) {
        printf("========== Some error has occured ==========\n");
...
AxialData &AxialData::LoadAxialData(const string &AxialFile_input) {
    ...
    while (!AxialFile.eof()) {
        ...
        if (inputString.empty()) getline(AxialFile, inputString);
    ...
    // 각 점의 좌표를 저장하는 변수
    pts = new double *[this->pts_num];
    for (size_t i = 0; i < this->pts_num; i++) pts[i] = new double[2];
    ...
    normal = new double *[this->pts_num];
    for (size_t i = 0; i < this->pts_num; i++) normal[i] = new double[2];

    // 각 점의 왼쪽, 오른쪽, 위쪽, 아래쪽의 index를 저장하는 변수
    EWNS_index = new int *[this->pts_num];
    for (size_t i = 0; i < this->pts_num; i++) EWNS_index[i] = new int[4];

    // 각 점을 포함하는 x-축선과 y-축선의 index를 저장하는 변수
    axial_index = new int *[this->pts_num];
    for (size_t i = 0; i < this->pts_num; i++) axial_index[i] = new int[2];

    // x-축선의 시작점과 끝점의 정보를 저장하는 변수 (시작점과 끝점의 y-좌표, 시작점의 x-좌표, 끝점의 x-좌표 순서로 저장)
    xaxial = new double *[Line_num_x];
    for (size_t i = 0; i < Line_num_x; i++) xaxial[i] = new double[3];

    // y-축선의 시작점과 끝점의 정보를 저장하는 변수 (시작점과 끝점의 x-좌표, 시작점의 y-좌표, 끝점의 y-좌표 순서로 저장)
    yaxial = new double *[Line_num_y];
    for (size_t i = 0; i < Line_num_y; i++) yaxial[i] = new double[3];
    ...
    while (!AxialFile.eof()) {
        ...
        // inputString의 크기가 0이라면 다시 한번 더 읽어서 inputString에 저장
        while (inputString.empty() && !AxialFile.eof()) getline(AxialFile, inputString);
        ...
        if (inputString.find('=') != string::npos) {
            ...
            switch (ind_int) {
                ...
                case 1:
                    ...
                    for (size_t i = Opts_num; i < in_pts_num + bd_pts_num; i++) {
                        AxialFile >> tmp >> pts[i][0] >> pts[i][1];
                        mp_u[i] = mp;
                    }
                ...
                case 3:
                    ...
                    while (Nx < nx) {
                        ...
                        if (this->bc_u[pt_previous] == 'C') {
                            sprintf(buf, "%s\n\t%s", "The first point of the x-axial line is the cross point",
                                    "Please check the Axial Data file");
                            PrintError(buf);
                        }
                ...
                case 4:
                    ...
                    while (Ny < ny) {
                        ...
                        // 읽어들인 첫번째 점이 내부점이라면 에러메시지를 출력
                        if (this->bc_u[pt_previous] == 'C') {
                            sprintf(buf, "%s\n%s", "The first point of the y-axial line is the cross point",
                                    "Please check the Axial Data file");
                            PrintError(buf);
                        }
...
//            this->normal[i][1] = -1.0E0, this->normal[i][0] = ZeroValue;
        ...
        if (Coordinate(x, y) == Coordinate(1.0E0, ZeroValue))
            this->normal[i][0] = -1.0E0, this->normal[i][1] = ZeroValue;
        if (Coordinate(x, y) == Coordinate(-1.0E0, ZeroValue))
            this->normal[i][0] = 1.0E0, this->normal[i][1] = ZeroValue;
        if (Coordinate(x, y) == Coordinate(ZeroValue, 1.0E0))
            this->normal[i][0] = ZeroValue, this->normal[i][1] = -1.0E0;
        if (Coordinate(x, y) == Coordinate(ZeroValue, -1.0E0))
            this->normal[i][0] = ZeroValue, this->normal[i][1] = 1.0E0;
        if (Coordinate(x, y) == Coordinate(2.0E0, ZeroValue))
            this->normal[i][0] = 1.0E0, this->normal[i][1] = ZeroValue;
        if (Coordinate(x, y) == Coordinate(-2.0E0, ZeroValue))
            this->normal[i][0] = -1.0E0, this->normal[i][1] = ZeroValue;
        if (Coordinate(x, y) == Coordinate(ZeroValue, 2.0E0))
            this->normal[i][0] = ZeroValue, this->normal[i][1] = 1.0E0;
        if (Coordinate(x, y) == Coordinate(ZeroValue, -2.0E0))
            this->normal[i][0] = ZeroValue, this->normal[i][1] = -1.0E0;
    }
