    ...
    if (pt->Condition() == 'D' && node >= adat->In_Pts_Num()) {
        ...
        if (!calc_pt)
            sprintf(errorMassage,
                    "SettingAzimuth with Dirichlet boundary, calc_pt = NULL point"), pt->PrintDebuggingData("11110",
                                                                                                            "SettingAzimuth"), PrintError(
                    errorMassage);
...
MatrixProcess &MatrixProcess::PrintDebuggingData(AxialData *adat, Point *pt, xData *xdat, yData *ydat) {
    ...
    if (*pt == 4005) {
        ...
        for (size_t i = 13; i < 26; i++)
            if (this->arrInt[i])
                printf("\n"),
                        printf("arrEnt[%02zu] * phi_ftn = %23.16e, ", i,
                               this->arrEnt[i] * u_ftn_Dirichlet(this->arrInt[i]->Phi())),
                        printf("arrEnt[%02zu] = %23.16e, ", i, this->arrEnt[i]),
    ...
    for (size_t i = 13; i < 26; i++)
        if (this->arrInt[i])
            result += this->arrEnt[i] * u_ftn_Dirichlet(this->arrInt[i]->Phi());
    ...
    printf("idx = %d/%d(%d), condition = %c, node = %d, result = %23.16e\n", pt->Index(), adat->Pts_Num(),
           adat->In_Pts_Num(), pt->Condition(), node, result);
